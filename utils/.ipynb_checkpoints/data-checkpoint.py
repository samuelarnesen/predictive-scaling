...
class CustomLogitFn(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        # You can perform any computation here that you want to 'remember'
        clipped_X = np.clip(X, 0.01, 0.99)
        next_X = np.log(clipped_X / (1 - clipped_X))

        self.mu_ = np.mean(next_X, axis=0)
        self.s_ = 1 / np.std(next_X - self.mu_, axis=0)
        
        # It's important to return self
        return self
    
    def transform(self, X):
        # Check if fit has been called
        check_is_fitted(self, ['mu_', 's_'])
        ...
        clipped_X = np.clip(X, 0.01, 0.99)
        X_transformed = np.log(clipped_X / (1 - clipped_X))
        return self.s_ * (X_transformed - self.mu_)
    ...
    def inverse_transform(self, X):
        ...
        # Apply the inverse logistic transformation
        rescaled = (X / self.s_) + self.mu_
        return 1 / (1 + np.exp(-rescaled))
        
    """
    def get_feature_names_out(self, input_features=None):
        # This method is useful for automatic feature naming in a pipeline
        if input_features is None:
            input_features = [f'x{i}' for i in range(self.mean_.shape[0])]
        return [f'{name}_powered' for name in input_features]
    """
...
def perform_pca(train_df, test_df=None, n_components=2, standardize=False, sparse=False, alpha=1, **kwargs):
    ...
    train_pc = pca_pipeline.fit_transform(train_scaled)
    ...
    if test_df is not None:
        test_pc = pca_pipeline.transform(test_scaled)
    ...
    return pca_pipeline.named_steps['pca'], train_principal_df, test_principal_df
...
def split_data(df, split_method, test_limit=None, cutoff_threshold=None, seed=42):
    ...
    elif split_method.startswith("model_family_cutoff_by_"):
        assert cutoff_threshold is not None, "Cutoff threshold is required for cutoff split"
        metric_name = "_".join(split_method.split("_")[4:])
        idx_max = df.groupby("Model Family")[metric_name].idxmax()
        test_df = df.loc[idx_max].reset_index(drop=True)
        train_df = df.drop(idx_max).reset_index(drop=True)
    else:
...
def fit_multivariate_regression_model(
    ...
    #nonlinearity="multidimensional" if X_train.shape[1] == 1 else nonlinearity
